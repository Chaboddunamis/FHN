
clear all 
tic
tmax=2000; % max time in seconds
step=0.2;       % time step in msec

%-----Cellular parameters ---------------
eps=0.02;
Vshift=-0.2;
alpha=1;
beta=0.01;


% Constant stimuli if any
gsyn12=0.02;
gsyn23=0.02;
gsyn31=0.02;

Erev=-2;

Iext=-0.1;
t1=600; % this time is in msec
t2=650; % this time is in msec
 

time=zeros(tmax/step,1);vv1=time;vv2=time; vv3=time;Caa1=time;Caa2=time; Caa3=time;
ss1=time;ss2=time; ss3=time;

V1= 2; V2= -2; V3= 2; Ca1=.61; Ca2=-0.6;  Ca3=1.6; s1=0; s2=0; s3=0;

tt=0;
i=0;
while (tt < tmax)
%pulse
 if tt>t1 && tt<t2
 Iapp=Iext;
 else
      Iapp=0.;
 end
    
V1 =V1 +step*( V1-V1.^3 - Ca1 + Iapp -gsyn31*s3*(V1-Erev));
V2 =V2 +step*( V2-V2.^3 - Ca2        -gsyn12*s1*(V2-Erev));
V3 =V3 +step*( V3-V3.^3 - Ca3        -gsyn23*s2*(V3-Erev));
Ca1=Ca1+step*(eps*(V1-Vshift-Ca1 ));
Ca2=Ca2+step*(eps*(V2-Vshift-Ca2));
Ca3=Ca3+step*(eps*(V3-Vshift-Ca3));
s1=s1  +step*(alpha*(1-s1)/(1+exp(-50*(V1))))-beta*s1;
s2=s2  +step*(alpha*(1-s2)/(1+exp(-50*(V2))))-beta*s2;
s3=s3  +step*(alpha*(1-s3)/(1+exp(-50*(V3))))-beta*s3;
tt=tt+step;
i=i+1;
time(i)=tt;
vv1(i)=V1;
vv2(i)=V2;
vv3(i)=V3;
Caa1(i)=Ca1;
Caa2(i)=Ca2;
Caa3(i)=Ca3;

ss1(i)=s1;
ss2(i)=s2;
ss3(i)=s3;

end  
 
toc     
figure(1)
clf
subplot(4,1,1)
plot(time,vv1,'Color',[0 0  .7],'LineWidth',1.5)
hold on
xlim([0 tmax]) 
ylim([-2 2])
xlabel('Time'),ylabel('Voltage')
 
subplot(4,1,2)
plot(time,vv2,'Color',[0 .7 0],'LineWidth',1.5)
hold on
plot(time,vv3*1.1,'Color',[.9 .0 0],'LineWidth',1.5)
hold on
xlabel('Time'),ylabel('Voltage')
xlim([0 tmax]) 
ylim([-2 2])
xlabel('Time'),ylabel('Voltage')
xlim([0 tmax]) 
ylim([-2 2])

subplot(4,1,3)
plot(time,Caa1,'Color',[0 0  .7],'LineWidth',1.5)
hold on
plot(time,Caa2,'Color',[0 .7  0],'LineWidth',1.5)
hold on
plot(time,Caa3*1.1,'Color',[0.8 .0  0],'LineWidth',1.5)
hold on
%ylim([0.5 1.05])
xlim([0 tmax])
xlabel('Time'),ylabel('[Ca]','Fontsize', 16)
 

 
subplot(4,1,4)
   plot(time,ss1,'Color',[0.0 0.0 0.7],'LineWidth',1.5)
   hold on
   plot(time,ss2,'Color',[0.0 0.6 0.0],'LineWidth',1.5)
   hold on
   plot(time,ss3,'Color',[0.8 0.0 0.0],'LineWidth',1.5)
   hold on
   ylim([0 1.1])
   xlim([0 tmax])
%   

figure(2)
clf
plot(Caa1,vv1,'Color',[0 0  1],'LineWidth',1.5)
hold on
  
V = [-2:0.1:2];
VN =V - V.^3 + -gsyn12*(V1-Erev);   % V-nullcline
plot (VN,V,'Color',[0.5 0.5 0.5])
hold on
VN = V - V.^3 + 0*Iext;   % V-nullcline
plot (VN,V,'Color',[0.2 0.2 0.2])
hold on
CaN=(V-Vshift); % Ca-nullcline
plot (CaN,V,'Color',[0.8 0.2 0.2])
hold on 
xlim([-1 1]) 
ylim([-2 2])
 xlabel('[Ca]-variable','Fontsize', 16),ylabel('Voltage','Fontsize', 16)