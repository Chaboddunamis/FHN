...
eps=0.1;
for I = [0.5:0.002:.565];  % Loop over different values of applied current I

f = @(t,y) [ (y(1) - y(1).^3 - y(2) + I); eps*(y(1) + 0.7 - 0.8*y(2) +0.07*sin(0.01*t) ) ];
f1 = @(t,y) [ -(y(1) - y(1).^3 - y(2) + I); -eps*(y(1) + 0.7 - 0.8*y(2)) ];
g = @(y) f(0,y);

fp = fsolve(g,[0 -0.5]);       % Find the fixed point
Vss = fp(1); Wss = fp(2);   % Get the steady-state V and W values from "fp"
J = [ [1 - Vss^2, -1]; [eps, -0.8*eps]];  % The Jacobian
...
[T,Y] = ode15s(f,(0:0.1:1450),[0,2],options);
