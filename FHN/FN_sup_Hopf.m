...
eps=0.2;
for I = [0.52:0.0001:.8];  % Loop over different values of applied current I

f = @(t,y) [ (y(1) - y(1).^3 - y(2) + I); eps*(y(1) + 0.7 - y(2)) ];
g = @(y) f(0,y);

fp = fsolve(g,[0 1]);       % Find the fixed point
Vss = fp(1); Wss = fp(2);   % Get the steady-state V and W values from "fp"
J = [ [1 - 3*Vss^2, -1]; [eps, -eps]];  % The Jacobian
...
[T,Y] = ode15s(f,(0:0.1:450),[-0.5,1],options);
...
[T Y] = ode15s(f, [0 400], Y(length(T),:) );  % Run some more in order to settle at equilibrium
